...
const {
  ...
  `course-certificates:${courseId.value}`,
  async () => await api.courseCertificates(courseId.value, {
...
useSeoMeta({
  title: () => (course.value ? `Zaświadczenia: ${course.value.name}` : 'Zaświadczenia kursu')
})
</script>
    ...
    >
      <div class="space-y-2">
        ...
        <h1 class="wrap-break-word text-3xl font-semibold tracking-tight text-slate-900">
          {{ course?.name || 'Zaświadczenia kursu' }}
        </h1>
        <p class="max-w-3xl text-sm leading-6 text-slate-600">
          Lista wydanych zaświadczeń dla wybranego kursu.
        </p>
      </div>
    ...
    <template v-else>
      ...
      <section class="rounded-xl border border-slate-200 bg-white/90 p-6 shadow-sm">
        ...
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 class="text-lg font-semibold text-slate-900">Wydane zaświadczenia</h2>
        ...
        >
          Nie udało się pobrać listy zaświadczeń dla tego kursu.
        </div>
        ...
        >
          Ładowanie wydanych zaświadczeń...
        </div>
        ...
        >
          Brak wydanych zaświadczeń dla tego kursu.
        </div>
